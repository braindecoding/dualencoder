...
class CLIPGuidedLoss(nn.Module):
    ...
    def __init__(self, device='cuda'):
        ...
        if CLIP_AVAILABLE:
            ...
            # Encode text prompts
            with torch.no_grad():
                text_tokens = clip.tokenize(self.digit_texts).to(device)
...
def evaluate_clip_guided_ldm():
    ...
    if CLIP_AVAILABLE:
        with torch.no_grad():
            ...
            for i in range(len(predictions)):
                ...
                # Calculate similarities with all digit texts
                similarities = torch.matmul(img_features, clip_loss_fn.text_features.T)
                predicted_digit = similarities.argmax().item()
    ...
    fig.suptitle('CLIP-Guided EEG LDM Results: Original vs Generated', fontsize=16)
    ...
    for i in range(4):
        if i < len(predictions):
            ...
            # Label info
            axes[3, i].text(0.5, 0.5, f'Target: {labels[i]}\nCorr: {correlations[i]:.3f}',
...
def main():
    ...
    print("=" * 70)
    print("Adding semantic guidance to ensure correct digit generation")
    print("=" * 70)
