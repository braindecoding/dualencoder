#!/usr/bin/env python3
"""
Improved CLIP-Guided EEG LDM with Better Semantic Accuracy
Enhanced approach with stronger CLIP guidance and classification loss
"""
...
class CLIPGuidedEEGDataset(Dataset):
    """Dataset for CLIP-guided EEG LDM training with labels"""
...
class ImprovedSemanticLoss(nn.Module):
    """Improved semantic loss with both CLIP and classification"""
    
    def __init__(self, device='cuda'):
        super().__init__()
        self.device = device
        
        # Classification head for direct digit prediction
        ...
        if CLIP_AVAILABLE:
            ...
            print(f"📊 Using {len(self.digit_text_templates)} text templates per digit")
        else:
            self.clip_model = None
            print("📊 Using classification loss only")
    
    def forward(self, generated_images, target_labels):
        """Calculate improved semantic loss"""
        ...
        if CLIP_AVAILABLE and self.clip_model is not None:
            ...
            # Combine both losses
            total_loss = classification_loss + 0.5 * clip_loss
            return total_loss, classification_loss, clip_loss
        else:
...
class ImprovedCLIPGuidedEEGDiffusion(nn.Module):
    """Enhanced EEG diffusion model with stronger semantic guidance"""
    ...
    def __init__(self, condition_dim=512, image_size=28, num_timesteps=100):
        ...
        print(f"📊 Improved CLIP-Guided EEG Diffusion Model:")
...
def train_improved_clip_guided_ldm():
    """Train improved CLIP-guided EEG LDM with stronger semantic guidance"""
    print("🚀 TRAINING IMPROVED CLIP-GUIDED EEG LDM")
    print("=" * 70)
    ...
    # Load datasets
    train_dataset = CLIPGuidedEEGDataset(split="train", target_size=28)
    test_dataset = CLIPGuidedEEGDataset(split="test", target_size=28)

    train_loader = DataLoader(train_dataset, batch_size=16, shuffle=True)
    test_loader = DataLoader(test_dataset, batch_size=16, shuffle=False)

    # Initialize models
    model = ImprovedCLIPGuidedEEGDiffusion(
        condition_dim=512,
        image_size=28,
        num_timesteps=100
    ).to(device)

    semantic_loss_fn = ImprovedSemanticLoss(device=device)
    ...
    mse_weight = 1.0
    l1_weight = 0.1
    semantic_weight = 2.0  # Increased semantic weight

    print(f"🎯 Improved CLIP-Guided Training Configuration:")
    print(f"   Epochs: {num_epochs}")
    print(f"   Batch size: 16")
    print(f"   Learning rate: 1e-4")
    print(f"   Loss weights: MSE={mse_weight}, L1={l1_weight}, Semantic={semantic_weight}")
    ...
    for epoch in range(num_epochs):
        ...
        for eeg_emb, images, labels in train_pbar:
            ...
            # Generate clean images for semantic loss
            with torch.no_grad():
            ...
            semantic_loss, class_loss, clip_loss = semantic_loss_fn(predicted_x0, labels)
        ...
        if avg_test_loss < best_test_loss:
            ...
            }, 'eeg_ldm_clip_improved_best.pth')
        ...
        if (epoch + 1) % 20 == 0:
            torch.save({
                'model': model.state_dict(),
                'semantic_loss': semantic_loss_fn.state_dict()
            }, f'eeg_ldm_clip_improved_epoch_{epoch+1}.pth')
    ...
    torch.save({
        'model': model.state_dict(),
        'semantic_loss': semantic_loss_fn.state_dict()
    }, 'eeg_ldm_clip_improved_final.pth')

    print(f"\n✅ Improved CLIP-guided training completed!")
    print(f"   Training time: {training_time:.2f} seconds")
    print(f"   Best test loss: {best_test_loss:.4f}")

    return model, semantic_loss_fn, train_losses, test_losses

def evaluate_improved_clip_guided_ldm():
    """Evaluate improved CLIP-guided EEG LDM with enhanced semantic accuracy"""
    print(f"\n📊 EVALUATING IMPROVED CLIP-GUIDED EEG LDM")
    print("=" * 60)
    ...
    checkpoint = torch.load('eeg_ldm_clip_improved_best.pth', map_location=device)
    ...
    print("🎨 Generating images with improved semantic guidance...")
    ...
    print(f"📊 Improved CLIP-Guided EEG LDM Performance:")
    print(f"   MSE: {mse:.4f}")
    ...
    # Visualize results with predictions
    fig, axes = plt.subplots(5, 4, figsize=(16, 20))
    fig.suptitle('Improved CLIP-Guided EEG LDM Results: Original vs Generated', fontsize=16)
    ...
    for i in range(4):
        if i < len(predictions):
            ...
            axes[3, i].text(0.5, 0.7, f'True: {labels[i]}', ha='center', va='center',
                           fontsize=14, transform=axes[3, i].transAxes)
            axes[3, i].text(0.5, 0.5, f'Pred: {predicted_digits[i]} {correct_symbol}',
                           ha='center', va='center', fontsize=14,
                           transform=axes[3, i].transAxes)
            axes[3, i].text(0.5, 0.3, f'Corr: {correlations[i]:.3f}',
                           ha='center', va='center', fontsize=12,
                           transform=axes[3, i].transAxes)
            axes[3, i].axis('off')

            # Accuracy summary
            axes[4, i].text(0.5, 0.5, f'Semantic Acc:\n{semantic_accuracy:.1%}',
                           ha='center', va='center', fontsize=12,
                           transform=axes[4, i].transAxes)
            axes[4, i].axis('off')

    plt.tight_layout()
    plt.savefig('eeg_ldm_clip_improved_results.png', dpi=300, bbox_inches='tight')
    plt.show()

    return predictions, targets, correlations, labels, semantic_accuracy, predicted_digits

def main():
    """Main function"""
    print("🎯 IMPROVED CLIP-GUIDED EEG LATENT DIFFUSION MODEL")
    print("=" * 80)
    print("Enhanced semantic guidance with classification + CLIP losses")
    print("=" * 80)

    # Train model
    _, _, _, _ = train_improved_clip_guided_ldm()

    # Evaluate model
    predictions, targets, correlations, _, semantic_accuracy, _ = evaluate_improved_clip_guided_ldm()

    print(f"\n🎯 IMPROVED CLIP-GUIDED EEG LDM SUMMARY:")
    ...
    print(f"\n📁 Generated files:")
    print(f"   - eeg_ldm_clip_improved_best.pth")
    print(f"   - eeg_ldm_clip_improved_final.pth")
    print(f"   - eeg_ldm_clip_improved_results.png")

    print(f"\n🚀 Improved CLIP Guidance Benefits:")
    print(f"   ✅ Enhanced semantic accuracy with classification head")
    print(f"   ✅ Multiple text templates for better CLIP guidance")
    print(f"   ✅ Stronger semantic loss weighting (2.0x)")
    print(f"   ✅ More training epochs (100 vs 50)")
    print(f"   ✅ Combined CLIP + classification losses")

    print(f"\n🧠 Expected Improvements:")
    print(f"   🎯 Higher semantic accuracy (target >50%)")
    print(f"   📈 Better digit classification")
    print(f"   🎨 Maintained visual quality")
