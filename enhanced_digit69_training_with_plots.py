...
def load_digit69_data():
    ...
    if 'training' in data:
        ...
        stimTest = data['test']['images']
        labelsTest = data['test']['labels']
    else:
        # Assume direct structure
        eegTrn = data['eeg_train']
        stimTrn = data['stim_train']
        labelsTrn = data['labels_train']

        eegTest = data['eeg_test']
        stimTest = data['stim_test']
        labelsTest = data['labels_test']
...
class EEGEncoder(nn.Module):
    """EEG Encoder for Digit69"""
    def __init__(self, n_channels=14, seq_len=256, embedding_dim=512):
        super().__init__()

        # Spatial projection
        self.spatial_proj = nn.Linear(n_channels, 64)
    ...
    def forward(self, x):
        ...
        x = self.spatial_proj(x)  # (batch, time, 64)
        x = x.transpose(1, 2)  # (batch, 64, time)
...
def run_enhanced_digit69_training():
    ...
    try:
        ...
        # Create EEG encoder
        eeg_encoder = EEGEncoder(n_channels=14, seq_len=256, embedding_dim=512).to(device)

        # Count parameters
        total_params = sum(p.numel() for p in eeg_encoder.parameters())
        print(f"🤖 EEG Encoder created: {total_params:,} parameters")

        # Run enhanced training
        history = enhanced_digit69_train_with_validation(
            eeg_encoder, train_loader, test_loader, device,
            epochs=100, lr=1e-3
        )
