...
class HybridCLIPSSIMLoss(nn.Module):
    ...
    def __init__(self, device='cuda'):
        ...
        if CLIP_AVAILABLE:
            ...
            print(f"📊 Using {len(self.letter_text_templates)} enhanced text templates per letter")
        else:
...
def evaluate_hybrid_clip_ssim_ldm():
    ...
    with torch.no_grad():
        ...
        predicted_letter_logits = hybrid_loss_fn.predict_digit(pred_tensor)  # Still called predict_digit but works for letters
        predicted_letters = predicted_letter_logits.cpu().numpy()

        correct_predictions = (predicted_letters == labels).sum()
    ...
    for i in range(4):
        if i < len(predictions):
            ...
            axes[0, i].set_title(f'Original {i+1}\nTrue Letter: {true_letter} ({labels[i]})', fontweight='bold')
            axes[0, i].axis('off')

            # Generated
            gen_img = predictions[i, 0]
            axes[1, i].imshow(gen_img, cmap='gray', vmin=-1, vmax=1)
            correct_symbol = "✅" if predicted_letters[i] == labels[i] else "❌"
            predicted_letter = letter_mapping[predicted_letters[i]]
            axes[1, i].set_title(f'Generated {i+1}\nPredicted: {predicted_letter} ({predicted_letters[i]}) {correct_symbol}', fontweight='bold')
            axes[1, i].axis('off')
            ...
            # Semantic metrics
            true_letter = letter_mapping[labels[i]]
            pred_letter = letter_mapping[predicted_letters[i]]
            axes[3, i].text(0.5, 0.8, f'True: {true_letter} ({labels[i]})', ha='center', va='center',
                           fontsize=14, fontweight='bold', transform=axes[3, i].transAxes)
            axes[3, i].text(0.5, 0.6, f'Pred: {pred_letter} ({predicted_letters[i]}) {correct_symbol}',
    ...
    return predictions, targets, correlations, labels, semantic_accuracy, predicted_letters, ssim_values
...
def main():
    ...
    print(f"   ✅ CLIP semantic guidance for correct letter generation")
